...
def query_3(
    session: SparkSession, logger: Logger, joinstrategy1: str, joinstrategy2: str
) -> Tuple[DataFrame, DataFrame]:
    """Load crime data, filter those in 2015 with a valid victim descent, join with revgecoding data to get ZIP
    codes, get top and bottom 3 zip codes based on income from the income data in 2015 and join with crime data to
    get the crimes in those zip codes, group by Victim Descent and count the number of crimes for each descent.
    ...
    crime_data = crime_data.filter(
        (year(col("DATE OCC")) == 2015) & (col("Vict Descent").isNotNull())
    )

    # reverse geocoding is read as doubles so we cast these
    logger.info("Casting LAT and LON to double")
    crime_data.withColumn("LAT", col("LAT").cast(DoubleType())).withColumn(
        "LON", col("LON").cast(DoubleType())
    )
    ...
    logger.info(
        "Keeping only the first ZIP code from the ZIP column in revgecoding data"
    )
    split_reg = "[^0-9]"
    ...
    crime_data_with_zip = crime_data.hint(joinstrategy1).join(
        revgecoding, ["LAT", "LON"], "inner"
    )
    ...
    income_2015 = income_2015.withColumn(
    ...
    crime_data_with_top_zip_n_income = crime_data_with_zip.hint(joinstrategy2).join(
        topZIPs, crime_data_with_zip["ZIPcode"] == topZIPs["Zip Code"], how="inner"
    )
    crime_data_with_top_zip_n_income.explain(
        extended=True
    )  # uses broadcast hash join here by default

    logger.info("Joining crime data with bottom 3 ZipCodes")
    crime_data_with_bottom_zip_n_income = crime_data_with_zip.hint(joinstrategy2).join(
        bottomZIPs,
        crime_data_with_zip["ZIPcode"] == bottomZIPs["Zip Code"],
        how="inner",
    )
    crime_data_with_bottom_zip_n_income.explain(
        extended=True
    )  # uses broadcast hash join here by default

    logger.info("Grouping by Victim Descent and counting the number of crimes")
    top_results = (
        crime_data_with_top_zip_n_income.groupBy("Vict Descent")
        .count()
    ...
    bottom_results = (
        crime_data_with_bottom_zip_n_income.groupBy("Vict Descent")
        .count()
        .orderBy("count", ascending=False)
        .select("Vict Descent", "count")
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="Join Strategy for LAT, LON with Zip Data",
    )
    parser.add_argument(
        "--join2",
        choices=["BROADCAST", "MERGE", "SHUFFLE_HASH", "SHUFFLE_REPLICATE_NL"],
...
if __name__ == "__main__":
    ...
    with spark(f"Query 3 ({joinstrategy1}, {joinstrategy2})") as session:
        ...
        with timer(
            f"Using join strategies: {'default' if joinstrategy1 == '' else joinstrategy1 } and {'default' if joinstrategy2 == '' else joinstrategy2}"
        ):
            top_results, bottom_results = query_3(
                session, logger, joinstrategy1, joinstrategy2
            )
